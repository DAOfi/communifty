...
contract ScorpioNFT is IScorpioNFT, ERC721 {
    ...
    mapping(uint256 => uint256) public override internalTokenToProjectId;
    mapping(uint256 => uint256) public override internalTokenToProjectTokenId;
    mapping(uint256 => uint256) public override projectToCurrentTokenId;
    mapping(uint256 => uint256) public override projectToMaxTokenId;
    mapping(uint256 => uint256) public override projectToMintPrice;
    mapping(uint256 => address) public override projectToRoyaltyAddress;
    mapping(uint256 => uint256) public override projectToRoyaltyFee;
    mapping(uint256 => uint256) public override projectToRoyalties;
    mapping(uint256 => uint256) public override projectToProceeds;
    mapping(uint256 => bool) public override projectToPreMint;
    mapping(uint256 => string) private _baseURIs;
    mapping(uint256 => string) private _tokenURIs;

    address public override owner;
    address public override proxyRegistryAddress;
    uint256 private _internalTokenId = 0;
    ...
    function disablePreMint(uint256 projectId_) external override onlyOwner {
    ...
    function preMint(
        ...
        require(
            projectToCurrentTokenId[projectId_].add(amount_) <=
                projectToMaxTokenId[projectId_],
            'MAX_SUPPLY'
        );
     ...
     */
    function _incrementTokenId(uint256 projectId_) private {
        _internalTokenId = _internalTokenId.add(1);
        internalTokenToProjectId[_internalTokenId] = projectId_;
        internalTokenToProjectTokenId[
            _internalTokenId
        ] = projectToCurrentTokenId[projectId_].add(1);
        projectToCurrentTokenId[projectId_] = internalTokenToProjectTokenId[
            _internalTokenId
        ];
    }
        ...
        returns (string memory)
    {
        ...
        uint256 projectId = internalTokenToProjectId[tokenId_];
        uint256 tokenId = internalTokenToProjectTokenId[tokenId_];
        return
            string(abi.encodePacked(_baseURIs[projectId], tokenId.toString()));
    }
     ...
     */
    function withdrawProceeds(uint256 projectId_) external override {
     ...
     */
    function withdrawRoyalties(uint256 projectId_) external override {
        payable(projectToRoyaltyAddress[projectId_]).transfer(
            projectToRoyalties[projectId_]
        );
        ...
        returns (uint256 tokenId_)
    {
        ...
        require(
            projectToCurrentTokenId[projectId_] <
                projectToMaxTokenId[projectId_],
            'MAX_SUPPLY_MINTED'
        );
        uint256 mintPrice = projectToMintPrice[projectId_];
        ...
        uint256 royaltyFee = mintPrice.mul(projectToRoyaltyFee[projectId_]).div(
            1000
        );
        projectToRoyalties[projectId_] = projectToRoyalties[projectId_].add(
            royaltyFee
        );
        // update proceeds
        projectToProceeds[projectId_] = projectToProceeds[projectId_]
            .add(msg.value)
            .sub(royaltyFee);
        emit Mint(
            projectId_,
